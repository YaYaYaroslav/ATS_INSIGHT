...
Евристичний парсер (без ML) — розбиває резюме на секції за заголовками,
а потім витягує skills/experience/education/projects/languages/certificates.

Це навмисно rule-based (не AI), щоб основний ATS-функціонал працював
навіть без Gemini/AI провайдера (вимога з архітектури проєкту).
"""
...
# --- Довідник навичок ---------------------------------------------------
# Невеликий, але розширюваний словник відомих технічних skills.
# У проді варто винести в БД/окремий конфіг і поповнювати динамічно.
KNOWN_SKILLS = {
...
def _split_into_sections(text: str) -> dict[str, str]:
    """
    Розбиває резюме на секції за рядками-заголовками (напр. "Experience",
    "Education"). Повертає {section_key: text_block}.
    Все, що до першого розпізнаного заголовка, йде в "header" (контакти, ім'я).
    """
    lines = text.split("\n")
    header_to_key = {}
    ...
    for line in lines:
        ...
        matched_key = None
        # Заголовок секції зазвичай короткий рядок, що співпадає зі словником
...
def _extract_skills(full_text_lower: str) -> list[str]:
    ...
    for skill in KNOWN_SKILLS:
        # межі слова, щоб "go" не спрацював всередині "google"
...
def _extract_name(header_block: str) -> str | None:
    """Дуже проста евристика: перший непорожній рядок без @ і цифр — ймовірно ім'я."""
...
def _extract_location(header_block: str) -> str | None:
    """Шукає рядок, що виглядає як 'Місто, Країна' у верхній частині резюме."""
    for line in header_block.split("\n")[:10]:
        line = line.strip()
...
def _extract_bullet_items(block: str) -> list[dict]:
    """
    Групує секцію (experience/education/projects) у список записів.
    Кожен запис — це рядок-заголовок (компанія/навчальний заклад) +
    список bullet-рядків під ним, доки не зустрінеться наступний заголовок.
    """
    items: list[dict] = []
    current_title = None
...
def _extract_languages(block: str) -> list[str]:
    if not block:
        return []
    # Розбиваємо по комах/переносах/крапках з комою
    raw = re.split(r"[,\n;]", block)
    return [r.strip() for r in raw if r.strip()]


def parse_resume(raw_text: str) -> dict:
    """Головна точка входу: сирий текст резюме -> структурований словник."""
