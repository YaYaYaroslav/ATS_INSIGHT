...
Ваги відповідають архітектурі проєкту:
Skills 35% / Experience 25% / Education 10% / Keywords 20% / Formatting 10%
"""
...
def calculate_skills_score(resume_skills: list[str], job_required: list[str], job_nice: list[str]) -> tuple[float, list[str], list[str]]:
    """Повертає (score 0-100, matched_skills, missing_skills)."""
    ...
    # Required skills важать більше за nice-to-have
...
def calculate_experience_score(resume_experience: list[dict], job_required_years: int | None) -> float:
    """
    Дуже спрощена евристика: рахує кількість записів experience як proxy
    для років досвіду (справжній парсинг дат — окрема задача).
    """
...
def calculate_formatting_score(resume_parsed: dict) -> float:
    """
    Проста перевірка "ATS-friendliness": чи є ключові поля розпізнані.
    Реальний ATS-formatting чекер аналізував би колонки, таблиці, зображення
    в PDF — тут спрощено до наявності структурованих даних.
    """
    checks = [
...
def run_full_analysis(resume_parsed: dict, resume_raw_text: str, job_parsed: dict) -> dict:
    """Головна точка входу: рахує всі скори та повертає готовий результат для збереження в Analysis."""
