...
class ScraperError(Exception):
    """Піднімається, коли не вдалось завантажити чи розпарсити сторінку вакансії."""
...
def _fetch_html(url: str) -> str:
    try:
        ...
        raise ScraperError(
            f"Сайт повернув помилку {exc.response.status_code}. Можливо, він блокує автоматичні запити."
        ) from exc
    except httpx.RequestError as exc:
        raise ScraperError(f"Не вдалося завантажити сторінку: {exc}") from exc
...
def _extract_jsonld_jobposting(soup: BeautifulSoup) -> dict | None:
    """Шукає <script type="application/ld+json"> з @type == JobPosting, включно з @graph-обгортками."""
...
def _extract_robota_ua(soup: BeautifulSoup) -> str | None:
    for selector in ["div[data-qa='vacancy-description']", "div.santa-mb-20", "article", "main"]:
...
def scrape_job_posting(url: str) -> dict:
    """Повертає {"title": str | None, "raw_text": str}."""
    ...
    domain = url.lower()
    site_text = None
    if "robota.ua" in domain:
        site_text = _extract_robota_ua(soup)
    elif "work.ua" in domain:
        site_text = _extract_work_ua(soup)
    ...
    if len(generic_text.strip()) < 100:
        raise ScraperError(
            "Не вдалося витягти достатньо тексту зі сторінки. Спробуй вставити опис вакансії вручну."
        )
